...
def print_data_base():
    ...
    col2 = 'И. Ф.'
    col3 = 'Предметы'
    col2_width = max([len(name) for name in g_name_list]) + 2
    col2_width = max(col2_width, len(col2)+2)
    col3_width = max([len(', '.join(s)) for s in g_subjects_list]) + 2
    col3_width = max(col3_width, len(col3) + 2)
    header = [col1.center(7), col2.center(col2_width),
              col3.center(col3_width)]
    print('\t'.join(header))
    for i in range(L):
        subjects = ', '.join(g_subjects_list[i])
        print(f'{g_id_list[i]}'.ljust(7),
              g_name_list[i].center(col2_width),
              subjects.center(col3_width), sep='\t')
